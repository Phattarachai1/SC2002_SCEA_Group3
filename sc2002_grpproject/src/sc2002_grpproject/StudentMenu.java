...
public class StudentMenu {
    ...
    private static boolean isEligible(Student student, Internship internship) {
        ...
        if (student.getYearOfStudy() <= 2 && internship.getLevel() != Internship.InternshipLevel.BASIC) {
            return false;
        }
        return true;
    }
    ...
    private static void applyForInternship(Student student, List<Internship> allInternships, 
        ...
        // Check eligibility
        if (!selectedInternship.isVisible() || selectedInternship.getStatus() != Internship.InternshipStatus.APPROVED) {
        ...
        if (selectedInternship.getStatus() == Internship.InternshipStatus.FILLED ||
    ...
    private static void requestWithdrawal(Student student, List<InternshipApplication> allApplications) {
        List<InternshipApplication> withdrawableApps = allApplications.stream()
            .filter(app -> app.getStudent().getUserID().equals(student.getUserID()))
            .filter(app -> app.getStatus() != InternshipApplication.ApplicationStatus.WITHDRAWN 
                        && app.getStatus() != InternshipApplication.ApplicationStatus.UNSUCCESSFUL)
