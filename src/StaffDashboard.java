...
public class StaffDashboard extends JFrame {
    ...
    private void refreshCompanyReps() {
        ...
        List<CompanyRepresentative> sortedReps = companyReps.stream()
            .sorted((r1, r2) -> {
                if (r1.getStatus() == CompanyRepresentative.ApprovalStatus.PENDING) return -1;
                if (r2.getStatus() == CompanyRepresentative.ApprovalStatus.PENDING) return 1;
                return 0;
            })
            .collect(Collectors.toList());
        
        for (CompanyRepresentative rep : sortedReps) {
            String statusText = rep.getStatus() == CompanyRepresentative.ApprovalStatus.APPROVED ? 
    ...
    private void authorizeRep() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            rep.setStatus(CompanyRepresentative.ApprovalStatus.APPROVED);
    ...
    private void approveWithdrawal() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            selected.setStatus(InternshipApplication.ApplicationStatus.WITHDRAWN);
    ...
    private void showFullReport() {
        ...
        report.append("Authorized Reps: ").append(companyReps.stream()
            .filter(r -> r.getStatus() == CompanyRepresentative.ApprovalStatus.APPROVED)
            .count()).append("\n");
        report.append("Pending Reps: ").append(companyReps.stream()
            .filter(r -> r.getStatus() == CompanyRepresentative.ApprovalStatus.PENDING)
            .count()).append("\n\n");
        
        report.append("Approved Internships: ").append(internships.stream().filter(i -> i.getStatus() == Internship.InternshipStatus.APPROVED).count()).append("\n");
        report.append("Pending Internships: ").append(internships.stream().filter(i -> i.getStatus() == Internship.InternshipStatus.PENDING).count()).append("\n\n");
        
        report.append("Pending Applications: ").append(applications.stream().filter(app -> app.getStatus() == InternshipApplication.ApplicationStatus.PENDING).count()).append("\n");
        report.append("Successful Applications: ").append(applications.stream().filter(app -> app.getStatus() == InternshipApplication.ApplicationStatus.SUCCESSFUL).count()).append("\n");
